#' ----
#' title: analysing cost-effective optimisation results - SCH
...
#' **NOTE 2**:
#' we ignored the results for "connectance_pa" for the 2505 EUR budget. We optimized all indicators systematically for the two budgets by cost/benefit but if the budget is high enough to optimize only by benefit, that is the result we need to take. So for "connectance_pa" 2505 EUR,- that is exactly the budget needed to optimize the indicator, therefore the cost benefit optimizations makes no sense and gives only a strange result. So we take the ecovalue from the ecol optimization (unconstrained budget) results here and not the cost benefit results (constrained budget). 
...
source('scripts/source_script.R') # my personal collection of functions
...
# original (unscaled) indicators (scores) to transform values to original scale
...
list(
  optis = read_csv("data/processed/single_ecol_sch.csv"),
  optis_2942 = read_csv("data/processed/single_ecol_econ_2942_sch.csv"),
  optis_2505 = read_csv("data/processed/single_ecol_econ_2505_sch.csv"),
  optis_scenarios = read_csv("data/processed/joint_ecol_sch.csv"),
  optis_scenarios_2942 = read_csv("data/processed/joint_ecol_econ_2942_sch.csv"),
  optis_scenarios_2505 = read_csv("data/processed/joint_ecol_econ_2505_sch.csv")
) %>% 
  map(
    ~ .x %>%
...
write.table(
  tbl_summary_costs_per_optis,
  "output/plots/economic_modelling/SCH/tbl_summary_costs_per_optis.txt",
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
ggsave(
  ...
  plot = last_plot(),
  path = "output/plots/economic_modelling/SCH/",
  dpi = "retina",
  height = 3.5,
  width = 3.5,
  units = "in",
...
# Repeat the same but for scenarios
...
ggsave(
  filename = "cost_per_opti_scenarios_across_all_sites_annotated.pdf",
  plot = last_plot(),
  path = "output/plots/economic_modelling/SCH/",
  dpi = "retina",
  height = 3.5,
...
# repeat the same for scenarios.. 

p_cost_by_sum_scenarios <- 
  optis_scenarios %>%
  summarise(sum_cost = unique(budget, na.rm = TRUE),
            sum_ecovalue = sum(ecovalue, na.rm = TRUE),
            .by = optimisation) %>% 
...
ggsave(
  filename = "cost_per_opti_scenarios_ranked_by_sum_ecoval.pdf",
  plot = last_plot(),
  path = "output/plots/economic_modelling/SCH/",
  dpi = "retina",
  height = 3.5,
...
# print both plots together for SCENARIOS
...
ggsave(
  filename = "costs_and_costs_ranked_by_sum_eco_val_scenarios.pdf",
  plot = last_plot(),
  path = "output/plots/economic_modelling/SCH/",
  dpi = "retina",
  height = 9,
...
optis %>%
  ...
  labs(x = "Indicator ranked by mean ecological value", 
       y = "Overall cost across all sites (euro)",
       # title = "Overall cost across all sites for ecological optimisation of single indicators ranked by the mean ecological value \nacross sites"
  )  +
  theme(plot.title.position = "plot")
...
write.table(tbl_sum_ecoval_per_indicator, 
            "output/plots/economic_modelling/SCH/tbl_sum_ecoval_per_indicator.txt",
            sep = ",",
            quote = FALSE,
            row.names = FALSE)
...
write.table(tbl_ratio_ecoval_per_indicator, 
            "output/plots/economic_modelling/SCH/tbl_ratio_ecoval_per_indicator.txt",
            sep = ",",
            quote = FALSE,
            row.names = FALSE)
...
optis %>% 
  summarise(
    n_sites = n(),
    total_cost = ceiling(sum(cost)), 
    .by = optimisation) %>% 
  arrange(total_cost)

# concl: the smallest budget (= overall cost per optimisation across all sites) was 2505 euros for connectance. We decided to use also the second smallest one, crop_pest_potential = 2942 €. With these two values, we did the economic-ecological (i.e. cost-effective) optimisation for constrained budget using 2505 and 2942. This step was done separately in C# programming language and the results were cleaned and loaded in ~/Documents/BTU-PhD/troph-cost/scripts/wrangling/data_tidying.R section 15. ECONOMIC MODELLING. 

## ---- 3.1) the sum of ecovalue reached in ecol-econ optimization using 2505€ compared to the max sum reachable per indicator in the ecol optimization
...
optis %>%
  ...
  left_join(
    optis_2505 %>% 
      summarise(
        sum_cost_2505 = sum(cost, na.rm = TRUE),
        sum_ecovalue_2505 = sum(ecovalue, na.rm = TRUE),
        .by = optimisation)) %>% 
  mutate(
    optimisation =
      optimisation %>%
  ...
  geom_segment(aes(x = optimisation, xend = optimisation, 
                   y = sum_ecovalue_2505, yend = sum_ecovalue), 
               color = "grey") +
  geom_point(aes(x = optimisation, y = sum_ecovalue_2505), 
             color = "gray25", size = 3) +
  ...
  geom_text(aes(x = optimisation, y = sum_ecovalue,
            ...
            size = 2,
            colour = "black") +
  geom_text(aes(x = optimisation, y = sum_ecovalue_2505, 
                label = round(sum_ecovalue_2505, digits = 1)), 
            stat = "identity", 
            nudge_y = -2,
            size = 2,
            colour = "black") +
  ...
  labs(x = "Optimisations ranked by max ecological value", 
       ...
       title = "Total ecovalue across sites using ecol-econ optimisation with 2505 euro \nbudget (filled circles) compared to ecol optimisation (emtpy circles)") +
  theme(plot.title.position = "plot")

# save figure 

ggsave(
  filename = "ecovalue_budget_2505.pdf",
  plot = last_plot(),
  path = "output/plots/economic_modelling/SCH/",
  dpi = "retina",
  height = 10,
...
## ---- 3.1) the sum of ecovalue reached in ecol-econ optimization using 2942€ compared to the max sum reachable per indicator in the ecol optimization
...
optis %>%
  ...
  left_join(
    optis_2942 %>% 
      summarise(
        sum_cost_2942 = sum(cost, na.rm = TRUE),
        sum_ecovalue_2942 = sum(ecovalue, na.rm = TRUE),
        .by = optimisation)) %>% 
  mutate(
    optimisation =
      optimisation %>%
  ...
  ggplot(.) +
  geom_segment(aes(x = optimisation, xend = optimisation, 
                   y = sum_ecovalue_2942, yend = sum_ecovalue), 
               color = "grey") +
  geom_point(aes(x = optimisation, y = sum_ecovalue_2942), 
             color = "gray25", size = 3) +
  geom_point(aes(x = optimisation, y = sum_ecovalue), 
             color = "black", size = 3, shape = 1) +
  geom_hline(data = optis %>%
  ...
  geom_text(aes(x = optimisation, y = sum_ecovalue,
            ...
            size = 2,
            colour = "black") +
  geom_text(aes(x = optimisation, y = sum_ecovalue_2942, 
                label = round(sum_ecovalue_2942, digits = 1)), 
            stat = "identity", 
            nudge_y = -2,
            size = 2,
            colour = "black") +
  ...
  labs(x = "Optimisations ranked by max ecological value", 
       y = "Sum of ecological value",
       title = "Total ecovalue across sites using ecol-econ optimisation with 2942 euro \nbudget (filled circles) compared to ecol optimisation (emtpy circles)") +
  theme(plot.title.position = "plot")

# save figure 

ggsave(
  filename = "ecovalue_budget_2942.pdf",
  plot = last_plot(),
  path = "output/plots/economic_modelling/SCH/",
  dpi = "retina",
  height = 10,
...
# Using the all_measures table, per indicator, rank the ecological value reached at each optimisation with scaled values from 0-1. Create a radar chart graph for each optimisation. In each radar chart show: 
...
# When creating the radar charts, we create the following objects for naming: 

# myfile_names = each (unique) optimisations (rows), independent of the budget. For naming each graph file.
# mytitle = each (unique) indicator (cols). Title for the graphs.
# vlabels = each (unique) indicator (cols). Names for each nodes in the graph. 
...
radar_charts_2505_2942 <-
  radar_charts_2505 %>%
  mutate(optimisation = str_c(optimisation, "_2505")) %>%
  bind_rows(radar_charts_2942 %>%
              mutate(optimisation = str_c(optimisation, "_2942"))) %>% 
  # relocate columns by groups of indicators
  relocate(
    optimisation, budget,
    # Traditional species conservation (TSC)
    butterfly_richness, plant_richness,  # red_list_butterflies, red_list_plants, # not in SCH
...
#' **NOTE 3** depending on the decisions for selecting measures previous the optimisation process, one could have the following case and should follow the steps mentioned: with the lowest budget we have ecol-econ optis csv files for all indicators except connectance, because the lowest budget comes from the ecological opti for that indicator. (So for connectance the ecol-econ opti result is the same as the ecol result). With the second lowest budget we have ecol-econ results only for 3 indicators and 3 scenarios. For the other indicators and scenarios we would not have a different result using the second-lowest budget and therefore we only show the result from the ecol-econ optis with the lowest budget. In the radar charts we will only have the results of the 3 indicators and 3 scenarios with both budgets and for all other indicators and scenarios we only show the results with the lowest budget. Therefore we shall remove them now from the "radar_charts_2505_2942" dataframe. 

# Uncomment the following chunk of code in case of having the scenario (or similar) of NOTE 3. 

# radar_charts_2505_2942_NA <-
#   radar_charts_2505_2942 %>%
#   rowwise() %>%
#   # make NA across all numeric variables where the row sum is = 0
#   mutate(across(where(is.numeric), ~ if_else(sum(c_across(where(is.numeric))) == 0, NA_real_, .))) %>%
#   ungroup() 

# keep only numeric vars in df for radar chart

radar_charts_num <-
  # radar_charts_2505_2942_NA %>% # use this in case of NOTE 3 scenario...
  radar_charts_2505_2942 %>% 
  column_to_rownames("optimisation")
...
radar_charts_num <- rbind(rep(100, 11) , rep(0, 11) , radar_charts_num) # TODO: change the number of repetitions  to the number of cols
radar_charts_num

# Prepare file names (i.e. the rows of radar_charts_num)

myfile_names <-
  radar_charts_2505_2942 %>% 
  select(optimisation) %>% 
  filter(str_detect(optimisation, "_2505"),
...
optis_scenarios_2505 %>% colnames()
optis_scenarios_2942 %>% colnames()

all.equal(
  optis %>% select(butterfly_richness:crop_pest_potential) %>% colnames(),
  optis_2505 %>% select(butterfly_richness:crop_pest_potential) %>% colnames(),
  optis_2942 %>% select(butterfly_richness:crop_pest_potential) %>% colnames(),
  optis_scenarios %>% select(butterfly_richness:crop_pest_potential) %>% colnames(),
  optis_scenarios_2505 %>% select(butterfly_richness:crop_pest_potential) %>% colnames(),
  optis_scenarios_2942 %>% select(butterfly_richness:crop_pest_potential) %>% colnames())

# concl: yes, we can recycle the vector...

# prepare variable labels (i.e. the column names of radar_charts_num)
vlabels_indicators <-
  radar_charts_num %>% 
  colnames() %>% 
...
for(i in 1:10) {
  ...
  mypath <-
    file.path("output/plots/economic_modelling/SCH/radar_charts_indicators", 
  ...
  radarchart(
    radar_charts_num[c(1:2,2+i, 12+i),], 
    axistype = 1,
...
pdf(file = "output/plots/economic_modelling/SCH/radar_charts_indicators/radar_chart_indicators_collage.pdf",
    width = 7.5, 
    height = 9.5)
...
radar_charts_summary <-
  ...
  mutate(opti = str_c(opti, "_2505")) %>%
  bind_rows(sum_budgets_2942 %>%
              rename(opti = optimisation) %>%
              mutate(opti = str_c(opti, "_2942"))
  ) %>% 
  bind_rows(
    radar_charts_2505 %>%
      mutate(opti = "ecoval_2505", .before = optimisation) %>%
      select(-budget) %>% 
      bind_rows(radar_charts_2942 %>%
                  mutate(opti = "ecoval_2942", .before = optimisation) %>% 
                  select(-budget)) %>% 
      pivot_longer(
        cols = butterfly_richness:last_col(),
        names_to = "indicator",
...
#' **NOTE 4**: following Note 3, we also remove from here all indicators with no values. BUT, visualisation in this way is a bit weird and the package fmsb does not allow for displaying floating (isolated) nodes, it always try to connect them to the centre of the graph. Therefore, and since it is only 3 indicators, we decided to annotate the values for 2942 budget manually based on the original values. 

# uncomment this chunk of code in case of scenario of NOTE 3 and 4

# radar_charts_summary_filt <-
#   radar_charts_summary %>% 
#   filter(!str_detect(opti, "_2942"))

# keep only numeric vars in df for radar chart
radar_charts_num <-
  # radar_charts_summary_filt %>% # use in case of NOTE 4
  radar_charts_summary %>% 
...
# To use the fmsb package, I have to add 2 lines to the dataframe: the max and min of each topic to show on the plot!
radar_charts_num <- rbind(rep(100, 10) , rep(0, 10) , radar_charts_num) # TODO: change the number of repetitions to the number of cols
radar_charts_num

pdf(file = "output/plots/economic_modelling/SCH/radar_charts_indicators/radar_chart_summary_indicators.pdf",
    width = 5.5, 
    height = 4)
...
radarchart(
  ...
  # custom polygon
  seg = 5, # n of segment per axis
  # pcol = c("#FDE332"), # color codes # use in case of NOTE 3 and 4
  pcol = c(hcl.colors(n = 2, palette = "viridis", alpha = 1, rev = T)), # color codes 
  pfcol = NULL, # color codes for filling polygons
  plwd = 0.8, # line widths 
  plty = 1 , # line types 
  # pty = c(18, 20), # point symbol # use in case of NOTE 3 and 4
...
# check that all dfs have the same number of optimisations 

nrow(optis_scenarios %>% distinct(optimisation))
nrow(optis_scenarios_2505 %>% distinct(optimisation))
nrow(optis_scenarios_2942 %>% distinct(optimisation))

# We have only 18 optimisations for ecol-econ optis for a 2942 budget 
...
sum_ecovalues_scenarios <-
  ...
  summarise(
    sum_ecol = sum(ecovalue[opti_type == "ecol"]),
    sum_ecol_econ_2505 = sum(ecovalue[opti_type == "ecol-econ-2505"]),
    sum_ecol_econ_2942 = sum(ecovalue[opti_type == "ecol-econ-2942"]),
    perc_ecoval_2505 = (sum_ecol_econ_2505 / sum_ecol) * 100,
    perc_ecoval_2942 = (sum_ecol_econ_2942 / sum_ecol) * 100,
    .groups = "drop"
  ) %>% 
  select(-c(sum_ecol_econ_2505, sum_ecol_econ_2942, sum_ecol)) %>% 
  # arrange df for fitting to radar chart format order by budget 
  pivot_longer(
    cols = perc_ecoval_2505:perc_ecoval_2942,
    names_to = "opti",
...
sum_budgets_scenarios <-
  ...
  summarise(
    sum_ecol = sum(budget[opti_type == "ecol"]),
    sum_ecol_econ_2505 = sum(budget[opti_type == "ecol-econ-2505"]),
    sum_ecol_econ_2942 = sum(budget[opti_type == "ecol-econ-2942"]),
    perc_budget_2505 = (sum_ecol_econ_2505 / sum_ecol) * 100,
    perc_budget_2942 = (sum_ecol_econ_2942 / sum_ecol) * 100,
    .by = optimisation
  ) %>% 
  select(-c(sum_ecol_econ_2505, sum_ecol_econ_2942, sum_ecol)) %>% 
  # arrange df for fitting to radar chart format order by budget 
  pivot_longer(
    cols = perc_budget_2505:perc_budget_2942,
    names_to = "opti",
    names_prefix = "perc_budget_",
    values_to = "percentage"
  ) %>% 
  arrange(as.numeric(opti)) %>% 
  pivot_wider(
    names_from = "optimisation",
    values_from = "percentage"
  ) %>% 
...
#' **NOTE 5**: if missing optimisations, follow the instructions of NOTE 3 and 4 for scenarios by uncommenting the following chunk of code

# radar_charts_scenarios_NA <-
#   radar_charts_scenarios %>%
#   # first we turn all inf values as 0
#   mutate(across(everything(), ~ ifelse(is.nan(.) | is.infinite(.), 0, .))) %>%
#   # then make NA across all numeric variables where the row sum is = 0
#   rowwise() %>%
#   mutate(across(where(is.numeric), ~ if_else(sum(c_across(where(is.numeric))) == 0, NA_real_, .))) %>%
#   ungroup()

# we turn all inf values as 0
...
radar_charts_num <-
  # radar_charts_scenarios_NA %>% # uncomment in case of NOTE 3, 4, and 5
  radar_charts_scenarios %>%
  column_to_rownames("optimisation")
...
for(i in 1:19) {
  ...
  # genetate paths for each image
  mypath <-
    file.path("output/plots/economic_modelling/SCH/radar_charts_scenarios", 
...
# assign paths for image
pdf(file = "output/plots/economic_modelling/SCH/radar_charts_scenarios/radar_chart_scenarios_collage.pdf",
    width = 7.5, 
    height = 9.5)
...
sum_ecovalues_scenarios_2505_2942 <- 
  # join all 3 dfs
  optis_scenarios %>%
  summarise(ecol = sum(ecovalue, na.rm = TRUE),
            .by = optimisation) %>%
  left_join(optis_scenarios_2505 %>%
              summarise(
                ecol_econ_2505 = sum(ecovalue, na.rm = TRUE),
                .by = optimisation
              )) %>%
  left_join(optis_scenarios_2942 %>%
              summarise(
                ecol_econ_2942 = sum(ecovalue, na.rm = TRUE),
                .by = optimisation
              )) %>%
  
  # calculate percentages
  mutate(
    perc_ecoval_2505 = (ecol_econ_2505 / ecol) * 100,
    perc_ecoval_2942 = (ecol_econ_2942 / ecol) * 100,
  ) %>%
  select(-c(ecol_econ_2505, ecol_econ_2942, ecol)) %>%
  
  # arrange data for radar chart
  pivot_longer(cols = perc_ecoval_2505:perc_ecoval_2942,
               names_to = "opti_type",
               values_to = "perc",
               names_prefix = "perc_ecoval_") %>% 
  pivot_wider(
...
sum_budget_scenarios_2505_2942 <-
  # join all 3 dfs
  optis_scenarios %>%
  ...
  left_join(optis_scenarios_2505 %>%
              summarise(
                ecol_econ_2505 = sum(cost, na.rm = TRUE),
                .by = optimisation
              )) %>%
  left_join(optis_scenarios_2942 %>%
              summarise(
                ecol_econ_2942 = sum(cost, na.rm = TRUE),
                .by = optimisation
              )) %>%
...
radar_charts_summary_scenarios <-
  sum_budget_scenarios_2505_2942 %>% 
  bind_rows(sum_ecovalues_scenarios_2505_2942)

# clean environment 

rm(sum_budget_scenarios_2505_2942, sum_ecovalues_scenarios_2505_2942)

#' **NOTE 6**: if needed, follow NOTE 4. Uncomment the code

# radar_charts_summary_filt <-
#   radar_charts_summary_scenarios %>% 
#   filter(!str_detect(opti_type, "_2942"))

# keep only numeric vars in df for radar chart

radar_charts_num <-
  # radar_charts_summary_scenarios_filt %>% if needed, follow NOTE 4. Uncomment the code
  radar_charts_summary_scenarios %>% 
...
# To use the fmsb package, I have to add 2 lines to the dataframe: the max and min of each topic to show on the plot!
radar_charts_num <- rbind(rep(100, 19) , rep(0, 19) , radar_charts_num) # TODO: change the number of repetitions to the number of cols
...
radarchart(
  ...
  # custom polygon
  seg = 5, # n of segment per axis
  # pcol = c("#FDE332"), # color codes # use in case of NOTE 5 and 6
  pcol = c(hcl.colors(n = 2, palette = "viridis", alpha = 1, rev = T)), # color codes 
  pfcol = NULL, # color codes for filling polygons
  plwd = 0.8, # line widths 
  plty = 1 , # line types 
  # pty = c(18, 20), # point symbol # use in case of NOTE 5 and 6
  pty = c(18, 18, 20, 20), # point symbol
...
sum_ecovalues_2505 <-
  ...
  summarise(
    across(.cols = butterfly_richness:crop_pest_potential,
           .fns = ~ sum(., na.rm = TRUE)),
    .by = optimisation) %>% 
  # add a row with the sum of ecovalue reach across indicators per optimisation 
  bind_rows(
    optis_2505 %>%
      summarise(
        maxvalue = sum(ecovalue, na.rm = TRUE),
        .by = optimisation) %>%
...
unique_budgets_indicators <- optis_2505 %>% 
...
sum_budgets_2505 <-
  as.data.frame(matrix(
    data = rep(unique_budgets_indicators$budget, 
               each = nrow(unique_budgets_indicators)),
    nrow = nrow(unique_budgets_indicators),
    byrow = TRUE
  ))

# Name the columns according to optimisations
colnames(sum_budgets_2505) <- unique_budgets_indicators$optimisation
rownames(sum_budgets_2505) <- unique_budgets_indicators$optimisation

sum_budgets_2505 <- sum_budgets_2505 %>% 
...
radar_charts_summary <-
  sum_ecovalues_2505 %>% 
  bind_rows(sum_budgets_2505_perc) %>% 
  # relocate columns by groups of indicators
  relocate(
    optimisation,
    # Traditional species conservation (TSC)
    butterfly_richness, plant_richness, #red_list_butterflies, red_list_plants, # no red_list indicators in region SCH
    # Regional distribution range (RDR)
    regional_distribution_butterflies, regional_distribution_plants, 
    # Trophic interactions (TIN)      
    trophic_interactions, unique_trophic_interactions, 
...
# keep anyway 0-100 to make radar charts comparable with each other
...
for(i in 1:10) {
  ...
  # genetate paths for each image
  mypath <-
    file.path("output/plots/economic_modelling/SCH/radar_charts_indicators/tradeoffs", 
...
# assign paths for image
pdf(file = "output/plots/economic_modelling/SCH/radar_charts_indicators/tradeoffs/radar_chart_tradeoffs_indicators_collage.pdf",
    width = 7.5, 
    height = 9.5)
...
# Name the columns according to scenarios for rows and indicators for cols
colnames(sum_budgets_scenarios_2505) <- unique_budgets_indicators$optimisation
rownames(sum_budgets_scenarios_2505) <- unique_budgets$optimisation

sum_budgets_scenarios_2505 <- sum_budgets_scenarios_2505 %>% 
  rownames_to_column(var = "optimisation") %>% 
  as_tibble()
...
# unlist the max values to use in division 

vec_unique_budgets <- unique_budgets_indicators %>% 
  pivot_wider(names_from = optimisation, values_from = budget) %>% 
  unlist()

sum_budgets_scenarios_2505_perc <-
  sum_budgets_scenarios_2505 %>%
  mutate(across(-optimisation, ~ (. / vec_unique_budgets[cur_column()])*100)) %>% 
  mutate(optimisation = paste(optimisation, "budget", sep = "_"))
...
radar_charts_summary <-
  sum_ecovalues_scenarios_2505 %>% 
  bind_rows(sum_budgets_scenarios_2505_perc) %>% 
  # relocate columns by groups of indicators
  relocate(
    optimisation,
    # Traditional species conservation (TSC)
...
trade_offs_scenarios_SCH <- radar_charts_summary

saveRDS(trade_offs_scenarios_SCH,
        file = 'data/processed/trade_offs_scenarios_SCH.rds')
...
for(i in 1:19) {
  ...
  # genetate paths for each image
  mypath <-
    file.path("output/plots/economic_modelling/SCH/radar_charts_scenarios/tradeoffs/", 
...
# assign paths for image
pdf(file = "output/plots/economic_modelling/SCH/radar_charts_scenarios/tradeoffs/radar_chart_tradeoffs_scenarios_collage.pdf",
    width = 7.5, 
    height = 9.5)
...
summary_tbl_indicators_sd %>% 
  ...
  # save table
  write.table(., 
              "output/plots/economic_modelling/SCH/tbl_mean_median_ecovalue_per_opti_tradeoffs_SCH.txt",
              sep = ",",
              quote = FALSE,
              row.names = FALSE)
...
#' [NOTE 7] Checking first our inverse_minmax function and replacing values by the original ones. These test were run before to check the calculations run without mistakes, but now are commented out. 

## ---- TEST 1 - one variable

# scaled_var <- norm_minmax(scores[scores$region == "SCH",]$butterfly_richness)
# orig_var <- scores[scores$region == "SCH",]$butterfly_richness
# undo_scale <- undo_minmax(scaled = scaled_var, unscaled = orig_var)
# 
# # Prove two are identical
# all.equal(scores[scores$region == "SCH",]$butterfly_richness, undo_scale)
# 
# # check function
...
#                     unscaled = scores[scores$region == "SCH",][[cur_column()]], 
#                     na.rm = TRUE)))

# check results 

# all.equal(test %>% select(butterfly_richness:red_list_plants, trophic_interactions:pollination_potential), 
#           scores %>% filter(region == "SCH") %>% select(butterfly_richness:red_list_plants, trophic_interactions:pollination_potential))
# 
# rm(test)
...
# all.equal(test, scores %>% filter(region == "SCH"))
# print(test)
# print(scores)
...
new_names <-
  c(
    ...
    "new_optis_2505",
    "new_optis_scenarios",
    "new_optis_scenarios_2505",
    "new_optis_scenarios_2942"
  )
...
write.table(tbl_summary_costs_and_ecovalue_per_optis_original_values, 
            "output/plots/economic_modelling/SCH/tbl_summary_costs_and_ecovalue_per_optis_original_values_SCH.txt",
            sep = ",",
            quote = FALSE,
            row.names = FALSE)
